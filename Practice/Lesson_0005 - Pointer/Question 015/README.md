# Write a C++ program using a class Flower to demonstrate:
  - Static array of objects
  - Dynamic array of objects
  - Use of setter function to assign values
  - Display object information
  
  - Also explain the destructor behavior.

```text
Static Array:
==============================
Enter Flower Name: Rose
Enter Flower Color: Red
Enter Flower Name: Tulip
Enter Flower Color: Pink

Flower ID: 0
Flower Name: Rose
Flower Color: Red
Flower ID: 1
Flower Name: Tulip
Flower Color: Pink


Dynamic Array:
==============================
Enter Flower Name: Lily
Enter Flower Color: White
Enter Flower Name: Daisy
Enter Flower Color: Yellow

Flower ID: 0
Flower Name: Lily
Flower Color: White
Flower ID: 1
Flower Name: Daisy
Flower Color: Yellow

Deleted: Flower - Tulip
Deleted: Flower - Rose
